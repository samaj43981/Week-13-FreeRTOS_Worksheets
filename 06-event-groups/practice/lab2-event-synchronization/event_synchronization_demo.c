...
void workflow_manager_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (xQueueReceive(workflow_queue, &workflow, portMAX_DELAY) == pdTRUE) {
            ...
            ESP_LOGI(TAG, "⏳ Waiting for workflow requirements (0x%08lX)...", required_events);
            EventBits_t bits = xEventGroupWaitBits(
                workflow_events,
                required_events,
...
void statistics_monitor_task(void *pvParameters) {
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "  Barrier events:   0x%08lX", xEventGroupGetBits(barrier_events));
        ESP_LOGI(TAG, "  Pipeline events:  0x%08lX", xEventGroupGetBits(pipeline_events));
        ESP_LOGI(TAG, "  Workflow events:  0x%08lX", xEventGroupGetBits(workflow_events));
    }
}
