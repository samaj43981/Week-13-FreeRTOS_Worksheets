...
// Forward declarations for pattern actions
...
void pattern_recognition_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (sensor_bits != 0) {
            ESP_LOGI(TAG, "🔍 Sensor event detected: 0x%08lX", sensor_bits);
            ...
            for (int p = 0; p < NUM_PATTERNS; p++) {
                ...
                for (int h = 0; h < EVENT_HISTORY_SIZE && pattern->required_events[event_index] != 0; h++) {
                    ...
                    if (record->event_bits & pattern->required_events[event_index]) {
                        event_index++;
                        ESP_LOGI(TAG, "✅ Pattern '%s': Found event %d/x (0x%08lX)", 
...
void state_machine_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (system_bits != 0) {
            ESP_LOGI(TAG, "🔄 System event: 0x%08lX", system_bits);
...
🏠 ═══ SMART HOME STATUS ═══");
        ...
        ESP_LOGI(TAG, "
📊 Event Group Status:");
        ESP_LOGI(TAG, "Sensor Events:     0x%08lX", xEventGroupGetBits(sensor_events));
        ESP_LOGI(TAG, "System Events:     0x%08lX", xEventGroupGetBits(system_events));
        ESP_LOGI(TAG, "Pattern Events:    0x%08lX", xEventGroupGetBits(pattern_events));
