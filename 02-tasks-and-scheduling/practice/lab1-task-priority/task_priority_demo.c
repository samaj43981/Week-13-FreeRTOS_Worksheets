...
void app_main(void)
{
    ...
    xTaskCreate(low_priority_task, "LowPrio", 3072, NULL, 1, &low_prio_handle);
    ...
    // xTaskCreate(priority_inversion_high, "PrioInvHigh", 2048, NULL, 5, NULL);
    // xTaskCreate(priority_inversion_low, "PrioInvLow", 2048, NULL, 1, NULL);

    // Exercise 1: Dynamic Priority
    xTaskCreate(dynamic_priority_demo, "DynamicPrio", 2048, low_prio_handle, 4, NULL);

    // Exercise 2: Task Affinity (example, not created by default)
    // xTaskCreatePinnedToCore(high_priority_task, "HighPrioCore0", 3072, NULL, 5, NULL, 0);
    // xTaskCreatePinnedToCore(low_priority_task, "LowPrioCore1", 3072, NULL, 1, NULL, 1);
